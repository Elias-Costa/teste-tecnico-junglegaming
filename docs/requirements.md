...
- **`FAILED` só em erro permanente de infraestrutura ou esgotamento para DLQ** — `[DECIDIDO: D-013]`. Erro transitório não altera o status. **O consumidor não escreve `FAILED`** — `[DECIDIDO: D-047]`: uma falha no consumo faz rollback da transação inteira e não deixa linha onde marcar, porque a transação é inserida já no estado terminal. **E `FAILED` acabou sem emissor nenhum** — `[DECIDIDO: D-064]`: D-047 previu que E-13 seria o emissor, ao esgotar o TTL da `PENDING_REFERENCE`, mas a §7.1 do enunciado escreve `REJECTED` para esse desfecho, e é o enunciado que vence (`AGENTS.md` §1). O status permanece no grafo, no `CHECK` do schema e na análise de D-050; os dois códigos de infraestrutura de D-007 ficam **reservados**. Consequência registrada como limitação conhecida: mensagem que vai à DLQ não deixa rastro consultável por RF-12.
...
- **A leitura acontece sob snapshot `REPEATABLE READ` e `read only`, sem lock** — `[DECIDIDO: D-057, D-065]`. Em READ COMMITTED, ler o saldo e depois o ledger veria dois instantes distintos, e uma aposta confirmada entre as duas leituras produziria divergência falsa num sinal que este requisito manda logar e contabilizar. D-057 resolvia isso com o `FOR UPDATE` de D-002, ao custo de bloquear o caminho do dinheiro daquela wallet; D-065 troca o mecanismo pelo snapshot e mantém `findByIdForUpdate` restrito ao processamento (RI-06). O `read only` é o que torna "divergência não é corrigida silenciosamente" uma recusa do banco (`25006`), e não uma promessa do código.
